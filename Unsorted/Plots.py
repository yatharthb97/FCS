...
TauD = GetCharResTime(3.0, param['veff_radius'], param['veff_radius'])

def FitFn(T):
    return 1 / ((1 + (T/TauD))* np.sqrt(1 + (T/TauD)/(param['veff_sf']*param['veff_sf'])))
...
plt.xscale("log")
plt.xlabel(r'$log_{10}(k)$ → $log_{10}(lag)$')
...
if do_ACF_Fit == True:
  ...
  plt.plot(acf_x, FitFn(acf_x*100*param['dt']*acf_y[0]), '--k', label = 'fit')
  plt.legend()
...
if param['do_pos_plots'] == True:
    ...
    xg, yg, zg = Get3DGauss(param['veff_radius'], param['veff_radius'], param['veff_radius']*param['veff_sf'])
    ...
    def graph_update(i):
      ...
      ax3d.plot_surface(xg, yg, zg, linewidth=0.0, cstride=1, rstride=1, alpha=0.1, color='yellow')
    ...
    ani = animation.FuncAnimation(fig_pos, graph_update, frames = np.arange(0, param['FrameExports']+1),blit = False, repeat = False, interval = 500) 
