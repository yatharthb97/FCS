...
import json
import datetime
import shapes #Contains the 3D Ellipsoid Generator
...
def plot_msd(t, msd, parent_path, show = False, fit=True):
  ...
  if fit:
    if param['PBC'] == False: #Plot Curve Fit
      ax.plot(t, msd_fit_fn(t), '--k', label = 'fit')

    else: #Plot Saturation Estimate
...
def plot_acf(acf_x, acf_y, polyfit=False, gauss_fit = True, draw_model=True, show = False):
  ...
  TauD = 0.25 * (param['veff_radius']**2) / param['D']
  ...
  #3.4 Save Plot
  acffigname = os.path.join(parent_path, 'msdplot.png')
...
def pos_plot_animation(draw_psf=True, save_vid=True, show=False):
    ...
    xg, yg, zg = shapes.Ellipsoid(param['veff_radius'], param['veff_radius'], param['veff_radius']*param['veff_sf'])
